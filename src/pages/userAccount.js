...
const userPhotoViaEmail = localStorage.getItem('profilePic') !== null;
export const userPhoto = userPhotoViaEmail ? localStorage.getItem("profilePic") : "http://localhost:3000/iconpg.jpeg"

const UserAccount = () =>{
    const userEmailLogin = auth?.currentUser?.email;
    const isRegisteredViaEmail = localStorage.getItem("email") !== null;
    const userEmail = isRegisteredViaEmail ? localStorage.getItem('email') : userEmailLogin;

    return (
    <div className="flex justify-center items-center h-screen">

    <div class="w-full max-w-sm overflow-hidden bg-white rounded-lg shadow-lg">
    <img class="object-cover object-center w-full" src={userPhoto ? userPhoto : "http://localhost:3000/iconpg.jpeg"} alt="avatar" />
    ...
    <div class="px-6 py-4">
        <div class="flex items-center text-gray-700">
            ...
            <h1 class="px-2 text-sm">{userEmail ? userEmail : userEmailLogin}</h1>
        </div>
        <Link to="/dashboard">
